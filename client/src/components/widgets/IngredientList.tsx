...
export function IngredientList({ inputs }: { inputs: CalculatedInputs }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
      {items.map((item) => (
        <div key={item.name} className="bg-secondary/30 rounded-xl p-2 sm:p-4 border border-white/5 flex flex-col items-center text-center hover:bg-secondary/50 transition-colors">
          <div className={`p-1.5 sm:p-2 rounded-full bg-background mb-2 sm:mb-3 ${item.color}`}>
            <item.icon className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
          <span className="text-xs sm:text-sm text-muted-foreground font-medium">{item.name}</span>
          <span className="text-xl sm:text-2xl font-bold mt-1">
            {item.value}<span className="text-xs sm:text-sm font-normal text-muted-foreground ml-1">{item.unit}</span>
          </span>
        </div>
      ))}
    </div>
